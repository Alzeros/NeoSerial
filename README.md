...
- **发送历史**:输入框手动发过的内容(`send-history.json`,最近 500 条,去重),补手册没收录的私有指令与带参数的完整命令。历史按前缀匹配、排在手册候选之前,与手册候选各自限量互不挤占;输入为空时按 ↑ 弹出历史列表(最近在前)。
...
| 文件 | 路径 | 说明 |
|------|------|------|
| 设置 | `%APPDATA%\neoserial\settings.json` | 串口默认值、UI 偏好、命令组、错误关键词、预设波特率、主题、MCP 设置、指令联想（知识库地址/Key、参与手册、弹出门槛与条数上限） |
...
```bash
cargo test           # Rust 单元测试（200 passed）
