...
- **知识库手册指令索引**:设置 → 指令联想 填知识库服务器地址与 API Key,"刷新指令库"按手册拉缓存到 `%APPDATA%/neoserial/command-index.json`(启动时可自动刷新),可勾选参与联想的手册。候选带中文名、语法、参数表、示例(以 AT 开头的示例行可点填入)、来源手册与页码;同名指令合并,"也见于"可切换其他手册的记录。
...
| 文件 | 路径 | 说明 |
|------|------|------|
| 设置 | `%APPDATA%\neoserial\settings.json` | 串口默认值、UI 偏好、命令组、错误关键词、预设波特率、主题、MCP 设置、指令联想（知识库地址/Key、参与手册、弹出门槛与条数上限、发送历史留存上限） |
