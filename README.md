...
- **知识库手册指令索引**:设置 → 指令联想 填知识库服务器地址与 API Key,"刷新指令库"按手册拉全量缓存到 `%APPDATA%/neoserial/command-index.json`(启动时可自动刷新),可勾选参与联想的手册。候选带中文名、语法、参数表、示例(以 AT 开头的示例行可点填入)、来源手册与页码;同名指令合并,"也见于"可切换其他手册的记录。
