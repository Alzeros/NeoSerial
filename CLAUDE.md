...
NeoSerial 启动后用配置端口(默认 34594)起 MCP server,并登记到 registry。
...
4. registry 缺失/全过期 → 从 34594 起逐端口 `get_status` 探测(兜底)。

## 配置 Claude Code 连接某实例
实例端口从 registry 拿到后(如 34594),配:
```
claude mcp add --transport http neoserial http://localhost:34594/mcp
```
或 .mcp.json:`{ "mcpServers": { "neoserial": { "type": "http", "url": "http://localhost:34594/mcp" } } }`
