...
<div class="relative h-full overflow-hidden flex flex-col" data-theme-target="background-data" style="background: var(--background-data);" oncontextmenu={handleContextMenu}>
  ...
  <!-- tabindex="-1":让容器能被鼠标点击聚焦,Ctrl+A 才知道该只圈日志区(范围逻辑在
       App.svelte 的 handleSelectAll)。用 -1 不进 Tab 序列,不干扰输入框键盘导航;
       outline-none 去掉点击聚焦后的焦点环(选中高亮已足够提示)。
       select-text:全局默认不可选,数据区显式开(见 app.css"文本选中范围")。 -->
  <div
