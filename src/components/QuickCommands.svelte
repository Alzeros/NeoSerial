<script lang="ts">
  ...
  const quickModules = $derived(
    scriptModules.filter((m) => isQuickCommandsModule(m)) as QuickCommandsModule[],
  );
  ...
  async function handleLoadConfig() {
    ...
    if (!path) return;
    try {
      const modules = await loadSequenceConfig(path, scriptModules as any);
...
<div class="flex h-full flex-col">
  ...
  {#if quickModules.length > 1}
  <div class="flex items-center gap-4 border-b border-[var(--border)] px-4 py-1" style="background: var(--background);">
    {#each quickModules as m}
      <!-- 必须取真实下标:过滤后 each 的 i 不再等于 scriptModules 的下标 -->
      {@const idx = scriptModules.indexOf(m)}
      <button
        class="text-[13px] font-medium transition-colors cursor-pointer pb-0.5 border-b-2 {idx === activeScriptModule.value
          ? 'text-[var(--foreground)] border-[var(--primary)]'
          : 'text-[var(--muted-foreground)] border-transparent hover:text-[var(--foreground)]'}"
        onclick={() => switchScriptModule(idx)}
      >
        {m.name}
      </button>
    {/each}
